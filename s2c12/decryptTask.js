...
function randomCrypto(pText) {
  ...
  let input=PKCS7(pText.concat(thing),16);
  let eRes;
...
function loopThat() {
  ...
  for (var i=numBlocks*16;i>=0;i-=16) {
    for (var j=16;j>0;j--) { //letter in the given block we're guessing, 1-16
      if (i===0&&j<=jStopIdx) { j=0; break; }
      let oneFewer=repeatN('A',(i+j-1));
      let oneShort=randomCrypto(oneFewer).slice(i,(numBlocks+1)*16).join("");
      for (var k=0;k<smartIter.length;k++) { //guess all possibilities, smartly
        ...
        postMessage({'type':'intermediate', 'res':str}); //post intermediate msg to the worker
        if (oneShort===randomCrypto(str).slice(i,(numBlocks+1)*16).join("")) {
