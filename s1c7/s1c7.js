...
//-->Encryption and Decryption Main Functions<--
  function aesEncrypt(plainText, key, numBits) {
    ...
    for (var i=0;i<plainText.length;i+=16) {
      temp.push(addRoundKey(plainText.slice(i,i+16),curKey)); }
    ...
    for (var i=0;i<totalRounds+1;i++) {
      ...
      if (i<totalRounds) {
        ...
        for (var j=0;j<currentState.length;j+=16) {
          ...
          temp.push(addRoundKey(mixColumns(sRow(bSub(state,true),true),true),curKey));
        }
        ...
        for (var j=0;j<currentState.length;j+=16) {
          let state=currentState.slice(j,j+16);
          temp.push(addRoundKey(mToBytes(sRow(bSub(state,true),true)),curKey));
        }
        currentState=flattenArray(temp);
      }
    }
  ...
  function aesDecrypt(cipherText, key, numBits) {
    ...
    for (var i=0;i<cipherText.length;i+=16) {
      ...
      temp.push(bSub(mToBytes(sRow(addRoundKey(state,curKey),false)),false));
    }
    let currentState=flattenArray(temp).map(e=>hexPad(e));;
    for (var i=0;i<totalRounds+1;i++) {
      ...
      if (i<totalRounds) {
        ...
        for (var j=0;j<currentState.length;j+=16) {
          let state=currentState.slice(j,j+16)
          let Z=mixColumns(bytesToM(addRoundKey(state,curKey)),false);
          Z=bSub(mToBytes(sRow(Z,false)),false);
          temp.push(Z);
        }
        currentState=flattenArray(temp).map(e=>hexPad(e));;
      }
      else {
        temp=[];
